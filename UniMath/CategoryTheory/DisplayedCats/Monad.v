...
Definition μ {D : disp_cat C} (TT : disp_monad_data D) := pr12 TT.
Definition η {D : disp_cat C} (TT : disp_monad_data D) := pr22 TT.
...
Definition disp_monad_mor {C : category} {D : disp_cat C} {S T : Monad C} (m : Monad_Mor S T) (SS : disp_monad S D) (TT : disp_monad T D) := ∑ a : disp_nat_trans m SS TT, disp_monad_mor_laws a.
...
Lemma monad_mor_η_nat_trans {C : category} {S T : Monad C} (f : Monad_Mor S T) : (nat_trans_comp _ _ _ (Monads.η S) f) = (Monads.η T).
Proof.
  apply nat_trans_eq.
  - apply homset_property.
  - apply Monad_Mor_η.
...
Lemma monad_mor_μ_nat_trans {C : category} {S T : Monad C} (f : Monad_Mor S T) : nat_trans_comp _ _ _ (Monads.μ S) f = nat_trans_comp _ _ _ (nat_trans_comp _ _ _ (whiskering.pre_whisker S f) (whiskering.post_whisker f T)) (Monads.μ T).
Proof.
  apply nat_trans_eq.
  - apply homset_property.
  - apply Monad_Mor_μ.
...
Proof.
  set (cl := (@cleaving_disp_functor_cat _ _ D D cleavingD (T : functor C C) (S : functor C C) (f : nat_trans _ _) (TT : disp_functor _ _ _))).
  ...
  - use (cartesian_factorisation (cartesian_lift_is_cartesian _ _ cl) (Monads.μ S)).
    apply (transportb (@mor_disp _ (disp_functor_cat C C D D) _ _ _ _) (monad_mor_μ_nat_trans f)).
  ...
  - use (cartesian_factorisation (cartesian_lift_is_cartesian _ _ cl) (Monads.η S)).
    exact (transportb (@mor_disp _ (disp_functor_cat C C D D) _ _ _ _) (monad_mor_η_nat_trans f) (η _ TT)).
Defined.

(*Ltac simpl_disp_cat := repeat (unfold transportb + rewrite transport_precomposition + rewrite transport_postcomposition + 
rewrite transport_f_f + rewrite id_left_disp + rewrite id_right_disp).*)
...
Proof.
  ...
  - apply (cartesian_factorisation_unique (cartesian_factorisation_disp_functor_cell_is_cartesian cleavingD TT f xx)).
    unfold transportb.
    rewrite transport_postcomposition.
    rewrite id_left_disp.
    rewrite assoc_disp_var.
    ...
    (cartesian_factorisation_disp_functor cleavingD TT f)
    (cartesian_factorisation_disp_functor cleavingD TT f)) TT _ (! monad_mor_μ_nat_trans f)).
    cbn.
    rewrite transport_precomposition.
    rewrite !assoc_disp.
    unfold transportb.
    ...
    rewrite !transport_f_f.
    rewrite cartesian_factorisation_commutes.
    rewrite (@transportf_disp_nat_trans _ D (functor_identity C) T _ _ _ _ (η T TT) (! monad_mor_η_nat_trans f)).
    rewrite !transport_postcomposition.
    etrans.
    {
      do 2 apply maponpaths.
...
Proof.
  set (cl := (@cleaving_disp_functor_cat _ _ D D cleavingD (T : functor C C) (S : functor C C) (f : nat_trans _ _) (TT : disp_functor _ _ _))).
  use make_disp_monad_mor.
  - exact (mor_disp_of_cartesian_lift _ _ cl).
  - split.
    + intros x xx.
      cbn.
      rewrite cartesian_factorisation_commutes.
      rewrite (transportf_disp_nat_trans (η T TT) (! monad_mor_η_nat_trans f)).
      apply transportf_paths.
      apply homset_property.
    + intros x xx.
      cbn.
      rewrite cartesian_factorisation_commutes.
      unfold transportb.
      rewrite (@transportf_disp_nat_trans _ D (S ∙ S) T _ _ (disp_functor_composite (cartesian_factorisation_disp_functor cleavingD TT f) (cartesian_factorisation_disp_functor cleavingD TT f)) TT _ (! monad_mor_μ_nat_trans f)).
      apply transportf_paths.
      apply homset_property.
Defined.
...
Proof.
  use make_disp_monad_mor.
  - set (cl := (@cleaving_disp_functor_cat _ _ D D cleavingD (T : functor C C) (S : functor C C) (f : nat_trans _ _) (TT : disp_functor _ _ _))).
    apply (cartesian_factorisation (cartesian_lift_is_cartesian _ _ cl) (g : nat_trans U S)).
    cbn.
    exact hh.
  - split.
    + intros x xx.
...
Definition disp_monad_cat_cartesian_lift {S T : Monad C} (f : Monad_Mor S T) (TT : disp_monad_disp_cat T) : cartesian_lift TT f.
Proof.
