...
Proof.
  ...
  - intros a b.
    ...
    + cbn.
      ...
      rewrite assoc.
      rewrite (assoc' _ (#(monoidal_cat_tensor Mon_V') _)).
      match goal with [|- ?f · _ · ?g = ?h] => apply (transportb (λ k, f · k · g = h) (nat_trans_ax (lax_monoidal_functor_μ F) _ _ (_ #, _))) end.
      simpl.
      rewrite assoc, assoc'.
      rewrite <- (functor_comp F).
      match goal with [|- ?f · _ = ?h] => apply (transportb (λ k, f · #F k = h) (enriched_id_right _ _)) end.
      apply pathsinv0.
      apply (lax_monoidal_functor_unital F).
  - intros a b c d.
    simpl in a, b, c, d.
