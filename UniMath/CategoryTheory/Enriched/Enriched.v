...
Local Definition enriched_nat_trans_comp_data {A B : enriched_precat} {F G H : enriched_functor A B} (a : enriched_nat_trans F G) (b : enriched_nat_trans G H) : enriched_nat_trans_data F H.
Proof.
  intro x.
  exact (a x · postcompose_underlying_morphism _ (b x)).
Defined.

Local Definition enriched_nat_trans_comp_law {A B : enriched_precat} {F G H : enriched_functor A B} (a : enriched_nat_trans F G) (b : enriched_nat_trans G H) : enriched_nat_trans_law (enriched_nat_trans_comp_data a b).
Proof.
  intros x y.
  unfold enriched_nat_trans_comp_data.
  simpl.
...
Proof.
  use make_enriched_nat_trans.
  - exact (enriched_nat_trans_comp_data a b).
  - apply enriched_nat_trans_comp_law.
Defined.
...
Proof.
  ...
  - split.
    + unfold enriched_cat_id, enriched_cat_mor, enriched_cat_comp.
      simpl.
  ...
  - intros a b c d.
    ...
    fold I.
    fold l_unitor.
    change (pr1 α) with (associator_to_nat_trans _ α).
    rewrite assoc.
    apply cancel_postcomposition.
    apply (transportb (λ f, # tensor (f #, id I) =
    pr1 α ((I, I), I) · # tensor (id I #, l_unitor I)) left_unitor_right_unitor_of_unit)%cat.
...
Proof.
  ...
  - intros t t' t''.
    induction t, t', t''.
    simpl.
    unfold enriched_cat_comp.
    simpl.
    apply (@pathscomp0 _ _ (# tensor ((id _ #, enriched_cat_id a) · (enriched_cat_id a #, id _)) · pr2 (pr2 (pr2 (pr1 A))) a a a) _)%cat.
    rewrite (functor_comp tensor).
    rewrite assoc'.
